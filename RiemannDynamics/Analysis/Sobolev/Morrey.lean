...
/-!
# Morrey embedding and Lusin (N) for super-critical Sobolev functions on `ℂ`

For `p > 2` a function `f : ℂ → ℂ` with weak gradient `(gx, gy) ∈ Lᵖ_loc` is
Hölder continuous of exponent `1 - 2/p` (Morrey), and maps null sets to null
sets (Lusin's condition (N)).
-/
