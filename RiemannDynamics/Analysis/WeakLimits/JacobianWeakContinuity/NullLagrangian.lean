...
`RiemannDynamics.Analysis.WeakLimits.JacobianWeakContinuity.WeakContinuity`.

## Main definitions

* `RiemannDynamics.partialX` — the partial `∂ₓ h z`, defined as the Fréchet directional
...
* `RiemannDynamics.partialY` — the partial `∂ᵧ h z`, the same construction in the direction
  `Complex.I`: `(fderiv ℝ h z) Complex.I`.
* `RiemannDynamics.jacobianWeak` — the Jacobian written in a pair `(gx, gy)` standing for
  `(∂ₓf, ∂ᵧf)`: `(gx z).re * (gy z).im − (gy z).re * (gx z).im`. Note the order in the
  subtracted term: it is `Re gy` times `Im gx`.
* `RiemannDynamics.TendstoWeaklyL2` — weak `L²(volume)` convergence `hₙ ⇀ h`, phrased as
...
* `RiemannDynamics.TendstoWeaklyL2Loc` — the same pairing convergence, but demanded only for
...
* `RiemannDynamics.det_fderiv_eq_partials` — for every `f` and `z`, with no hypotheses,
...
* `RiemannDynamics.integral_jacobian_smul_eq` — the null-Lagrangian integration-by-parts
...
* `RiemannDynamics.integral_jacobianWeak_smul_eq` — the same identity with the weak
...
* `RiemannDynamics.partialX_def`, `RiemannDynamics.partialY_def` — the two `rfl` unfolding
...
* `RiemannDynamics.jacobianWeak_def` — the `rfl` unfolding lemma for `jacobianWeak`.
* `RiemannDynamics.TendstoWeaklyL2.tendstoWeaklyL2Loc` — global weak `L²` convergence implies
  the compactly-supported-test form, by restricting the class of test functions.
* `RiemannDynamics.TendstoWeaklyL2Loc.comp_strictMono` — local weak `L²` convergence is
