...
`RiemannDynamics.Analysis.WeakLimits.JacobianWeakContinuity.NullLagrangian`. Because
...
* `RiemannDynamics.tendsto_integral_jacobianWeak_smul` — weak continuity of the tested
...
* `RiemannDynamics.hasWeakDirDeriv_of_tendsto` — if `fₙ → g` locally uniformly with every `fₙ`
...
* `RiemannDynamics.le_liminf_integral_normSq_smul` — weighted weak lower semicontinuity of the
...
* `RiemannDynamics.le_liminf_integral_normSq_smul_of_compactSupport` — the same inequality
