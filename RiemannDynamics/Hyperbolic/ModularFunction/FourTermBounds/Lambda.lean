...
/-!
# Four-term q-expansion bounds: the modular lambda and the widened variants

* `modularLambdaH_norm_sub_four_term_le_of_im_ge_one` — the four-term truncation bound
  `‖λ − 16q + 128q² − 704q³ + 3072q⁴‖ ≤ 131072·exp(−5π·τ.im)` on `τ.im ≥ 1`.
* Widened four-term bounds on `τ.im ≥ 9/10` for `jacobiTheta₂ (τ/2) τ`, `θ₂`, and `θ₃`
  (with the lower bound `theta3_norm_ge_half_of_im_ge_nine_tenths`) and for `λ`
  (`modularLambdaH_norm_sub_four_term_le_of_im_ge_nine_tenths`, with the tighter
