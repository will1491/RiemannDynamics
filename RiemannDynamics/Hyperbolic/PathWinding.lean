...
* `continuous_log_lift_param_of_continuous_ne_zero`: the 2D-parametric
  extension — a jointly continuous log lift over a rectangle.

* `pathContourIntegral_inv_eq_log_lift_diff_of_contDiff` (with the
  `_of_contDiffOn` and `_F_Y_image_curve` variants): FTC bridges
  identifying the contour integral of `(z − w)⁻¹` along a path with
  the boundary difference of any continuous log lift of `γ − w`.
