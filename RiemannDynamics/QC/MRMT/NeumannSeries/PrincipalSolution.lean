...
* `RiemannDynamics.IsPrincipalSolution` — `IsPrincipalSolution b f` records the Neumann-series
...
* `RiemannDynamics.hasWeakGradient_cauchyTransform` — for `2 < p`, `p ≠ ⊤`, `MemLp h p volume`
...
* `RiemannDynamics.exists_lp_fixedPoint_beltrami` — for `2 < p`, `p ≠ ⊤`, measurable `μ` with
...
* `RiemannDynamics.exists_isPrincipalSolution` — a Beltrami coefficient vanishing outside a ball
...
* `RiemannDynamics.IsPrincipalSolution.continuous` — a principal solution is continuous on `ℂ`.
* `RiemannDynamics.IsPrincipalSolution.tendsto_sub_id_cocompact` — the principal normalization:
  `f z − z → 0` along `cocompact ℂ`.
* `RiemannDynamics.IsPrincipalSolution.hasWeakGradient` — a principal solution has weak partials
...
* `RiemannDynamics.IsPrincipalSolution.memW12loc` — a principal solution is `W^{1,2}_loc` on `ℂ`.
* `RiemannDynamics.IsPrincipalSolution.weak_beltrami` — a principal solution has a weak gradient
...
* `RiemannDynamics.isPrincipalSolution_unique` — two principal solutions of the same coefficient
