...
* `RiemannDynamics.memLp_of_eLpNormEssSup_ne_top_of_support` — a measurable `μ` with
...
* `RiemannDynamics.integrable_div_sub_of_memLp_of_support` — for `2 < p`, `p ≠ ⊤`,
...
* `RiemannDynamics.cauchyTransform_sub_le_holder` — under the same four hypotheses there is a
...
* `RiemannDynamics.continuous_cauchyTransform_of_memLp_of_support` — the same hypotheses give
  `Continuous (cauchyTransform h)`; the qualitative reading of the Hölder bound, and the form
  nearly all call sites use.
* `RiemannDynamics.cauchyTransform_tendsto_cocompact` — under the same hypotheses
