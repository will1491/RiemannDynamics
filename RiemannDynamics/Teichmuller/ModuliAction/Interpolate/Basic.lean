...
/-!
# Interpolation, I: words, relation killing, correction paths, and the collar estimate

The head of the interpolation engine for Marden stability: evaluation of words in a
generator tuple with membership, closure extraction, and entrywise convergence; relation
killing under a trace gap (`eventually_eq_pm_one_of_tendsto_pm_one`,
`eventually_eq_up_to_sign_of_tendsto`); the determinant-normalized correction path
`correctionPath` with entrywise smoothness and size bounds; the Möbius map `matMoebius`
of a real `2 × 2` matrix; and the collar transition estimate
`collar_correction_estimate`.
-/
